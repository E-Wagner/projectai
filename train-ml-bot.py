...
# IMPORTANT
# bot you want to train against:
from bots.ml_rdeep import ml_rdeep as opponent
# bot you want to train the model for
from bots.ml_ml.ml_ml import features
# second part of the name of the bot you're training for
pname = 'ml'


def create_dataset(path, player=opponent.Bot(), games=2000, phase=1):
    data = []
    target = []
...
parser.add_argument("-m", "--model-path",
                    dest="model_path",
                    help="Optional model path. Note that this path starts in bots/ml_rdeep/ instead of the base folder, like "
                         "dset_path above.",
                    default="model.pkl")
...
parser.add_argument("-o", "--overwrite",
                    dest="overwrite",
                    action="store_true",
                    help="Whether to create a new dataset regardless of whether one already exists at the "
                         "specified path.")
...
if options.overwrite or not os.path.isfile(options.dset_path):
    create_dataset(options.dset_path, player=opponent.Bot(), games=10000)
...
if options.train:
    ...
    # Store the model in the ml_{opponent} directory
    joblib.dump(model, "./bots/ml_" + pname + "/" + options.model_path)
